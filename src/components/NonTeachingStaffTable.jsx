import { FaGithubSquare, FaLinkedin } from "react-icons/fa";

const nonTeachingStaffData = [
  {
    name: "Vinay Kumar",
    designation: "Accountants",
    image: "/images.png",
    githubLink: "",
    linkedInLink: "",
  },
  {
    name: "Sikandar Thakur",
    designation: "Store Keeper",
    image: "/images.png",
    githubLink: "",
    linkedInLink: "",
  },
  {
    name: "Jitendra Kumar Rajak",
    designation: "Librarian",
    image: "/images.png",
    githubLink: "",
    linkedInLink: "",
  },
];

const NonTeachingStaffTable = () => {
  return (
    <div className="bg-gray-100 py-3 lg:py-5 px-2 lg:px-6">
      <h2 className="text-2xl lg:text-3xl font-bold text-center text-blue-900 mb-2 lg:mb-10">
        Non-Teaching Staff List
      </h2>

      <div className="overflow-x-auto">
        <div className="max-w-6xl mx-auto bg-white rounded-xl shadow-md overflow-hidden min-w-145">
          {/* Header */}
          <div className="grid grid-cols-3 bg-gray-200 text-gray-700 font-semibold px-2 lg:px-6 py-2 lg:py-3 text-sm">
            <div className="whitespace-nowrap">STAFF MEMBER</div>
            <div className="whitespace-nowrap">DESIGNATION</div>
            <div className="whitespace-nowrap">PROFILE LINK</div>
          </div>

          {/* Rows */}
          {nonTeachingStaffData.map((staff, index) => (
            <div
              key={index}
              className="grid grid-cols-3 items-center px-2 lg:px-6 py-2 lg:py-4 border-t border-t-gray-400 hover:bg-gray-50"
            >
              {/* Staff */}
              <div className="flex items-center gap-1 lg:gap-3 whitespace-nowrap">
                <img
                  src={staff.image}
                  alt={staff.name}
                  className="w-10 h-10 rounded-full object-cover"
                />
                <span className="font-medium lg:text-lg text-sm">
                  {staff.name}
                </span>
              </div>

              {/* Designation */}
              <div>
                <span className="bg-blue-100 text-xs lg:text-sm text-emerald-800 px-1 font-medium py-0.5 rounded border whitespace-nowrap">
                  {staff.designation.toUpperCase()}
                </span>
              </div>

              {/* Links */}
              <div className="flex items-center gap-3 whitespace-nowrap">
                {/* GitHub */}
                <a
                  href={staff.githubLink || "#"}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`flex items-center gap-1 border px-1 py-0.5 rounded text-xs lg:text-sm font-medium
              ${
                staff.githubLink
                  ? "text-gray-800 hover:bg-gray-200 border-gray-400"
                  : "text-gray-400 border-gray-300 cursor-not-allowed pointer-events-none"
              }`}
                >
                  <FaGithubSquare className="text-2xl" />
                  GitHub
                </a>

                {/* LinkedIn */}
                <a
                  href={staff.linkedInLink || "#"}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`flex items-center gap-1 border px-1 py-0.5 rounded text-xs lg:text-sm font-medium
              ${
                staff.linkedInLink
                  ? "text-gray-800 hover:bg-gray-200 border-gray-400"
                  : "text-gray-400 border-gray-300 cursor-not-allowed pointer-events-none"
              }`}
                >
                  <FaLinkedin className="text-2xl text-blue-800" />
                  LinkedIn
                </a>
              </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default NonTeachingStaffTable;
